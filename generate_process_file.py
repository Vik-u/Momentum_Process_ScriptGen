#!/usr/bin/env python3
"""
Template-based Momentum process file generator.

It keeps the static header (devices, variables, pools) from a base Momentum DB
file and replaces only the process section with user-selected steps. Each step
is populated with provided overrides, otherwise falls back to observed defaults
from the catalog, otherwise uses <FILL>.
...
def build_process_block(
    catalog: Dict, steps_spec: List[Tuple[str, str, Dict[str, str], str]], process_name: str
) -> List[str]:
    def esc(val: str) -> str:
        return val.replace("'", "''")
    ...
    for device, op, overrides, cont_override in steps_spec:
        ...
        out.append("")  # spacer line between steps
...
def main() -> None:
    ...
    parser.add_argument(
        "--steps",
        required=True,
        help="Step list like \"Device:Operation;param=value|Device2:Operation\". Use '|' to separate steps.",
    )
    parser.add_argument("--output", default="generated_process.txt", help="Where to write the generated file.")
    ...
    process_block = build_process_block(catalog, steps_spec, args.process_name)

    output_lines = header + process_block
    Path(args.output).write_text("\n".join(output_lines) + "\n")
    print(f"Wrote process file to {args.output}")
